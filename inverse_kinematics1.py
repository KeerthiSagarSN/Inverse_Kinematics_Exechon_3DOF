...
OS_subs = S_x*O.i + S_y*O.j + S_z*O.k

#OS_subs =  0.5*O.j + 0.75*O.k

# Projected point on the ik plane
t_0 = sqrt(pow(S_x,2) + pow(S_z,2))

# Determining the plane pi_alfa


c_alpha1 = (-1*S_x)/(t_0*1.0)
s_alpha1 = (S_z)/(t_0*1.0)

c_alpha2 = ((sqrt(pow(t_0,2) -  pow(h_x,2)))/(t_0*1.0))
s_alpha2 = (h_x/(t_0*1.0))

#alpha = alpha1 + (delt_1_B*alpha2)



c_alpha = cos(acos(c_alpha1)+delt_1_B*acos(c_alpha2))
s_alpha = sin(asin(s_alpha1)+delt_1_B*asin(s_alpha2))


# Unit vector along the axis of Joint 2 and Joint 4 and Joint 5
k_2_A = (s_alpha)*O.i + (c_alpha)*O.k
k_2_C = k_2_A

# Get vector k_5_B by finding the plane pi_e_b

P_1_B_S = OS_subs - OB_subs

# Get k_5_B

k_5_B_pi = k_2_A.cross(P_1_B_S)

k_5_B = k_5_B_pi/k_5_B_pi.magnitude() # Unit vecor


# Locate the new reference frame "P"

P = O.locate_new('P', O.i + O.j + O.k )
...
alpha2 = acos((PO_S_subs.dot(-1*O.i))/PO_S_subs.magnitude())


#O_PO_S_subs_k = O_PO_S*(O.k)



n_PO_S_subs = OS_subs/OS_subs.magnitude()

OSP_subs = OS_subs + (hz)*n_PO_S_subs
...
O_PO_S_OS_subs =  OB_subs - O_PO_S_subs
v_norm1_BS_OS = BS_subs.cross(O_PO_S_OS_subs)

# Perpendicular vector between OSP and v_ra

#v_norm1_subs = v_norm1_BS_OS
v_norm1_subs = v_norm1_BS_OS.cross(BS_subs)

#v_norm1_subs = v_norm2_subs.cross(v_norm1_BS_OS)
...
plot_line(get_vector(O_PO_S_subs,O),get_vector(OS_subs,O),'k') 
